...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public class PostProcessPass : ScriptableRenderPass
    {
        ...
        public void SetupFinalPass(in RenderTargetHandle source, bool enableSRGBConversion = true, bool useRenderColorBuff = false)
        {
            m_Source = source;
            m_Destination = RenderTargetHandle.CameraTarget;
            m_IsFinalPass = true;
        ...
        void Render(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.UberPostProcess)))
            {
                ...
                RenderTargetIdentifier targetDestination =  destination ;
...
//             }
//
//             void Swap() => CoreUtils.Swap(ref source, ref destination); //-->这玩意儿在12里已经化成了ColorBufferSystem  用一张纹理交换
//
//             // Setup projection matrix for cmd.DrawMesh()
//             cmd.SetGlobalMatrix(ShaderConstants._FullscreenProjMat, GL.GetGPUProjectionMatrix(Matrix4x4.identity, true));
//
...
//         }
        ...
        void RenderFinalPass(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            // {
            //     m_Source.id = cameraData.renderer.cameraColorTarget;
            // }
            if (m_FinalPassUseRenderColorBuff)
            {
                var renderer = renderingData.cameraData.renderer as ForwardRenderer;
                var colorBuffer = renderer.m_ColorBufferSystem;
                m_Source = colorBuffer.GetBackBuffer();
