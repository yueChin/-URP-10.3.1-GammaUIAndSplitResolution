...
#pragma warning restore 414
        ...
        public override void Setup(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            if (lastCameraInTheStack)
            {
                ...
                //Debug.LogError($"最後一個： {applyPostProcessing}");
                ...
                //Debug.LogError($"最後一個2： {cameraTargetResolved}");
            ...
            else if (applyPostProcessing)
            {
                ...
                if (sUISplitEnable && cameraData.nextIsUICamera)
                {
                    ...
                    if (applyFinalPostProcessing)
                    {
                        ...
                        m_FinalPostProcessPass.SetupFinalPass(in sourceForFinalPass, false,true);
                        EnqueuePass(m_FinalPostProcessPass);
                    }
...
//             }
        ...
        public void ResizeDepth(CommandBuffer cmd, ref RenderTextureDescriptor descriptor,int width,int height)
        {
            if (m_ActiveCameraDepthAttachment != RenderTargetHandle.CameraTarget)
            {
                ...
                // binding MS surfaces is not supported by the GcameraData.requireSrgbConversion);LES backend, and it won't be fixed after investigating
                // the high performance impact of potential fixes, which would make it more expensive than depth prepass (fogbugz 1339401 for more info)
                if (IsGLESDevice())
                    depthDescriptor.bindMS = false;
        ...
        {
            //Debug.LogError($"當前是不是UI sUISplitEnable :  {sUISplitEnable}      sIsGammaCorrectEnable : {sIsGammaCorrectEnable}  isUICamera : {cameraData.isUICamera}   ");
